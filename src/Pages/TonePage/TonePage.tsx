...
class TonePage extends React.Component<{
  ...
  render() {
    ...
    const tt = (key: string, opt?: any) => i18n.t('tones:tone' + num + key, opt);
    return (
      <div className="tone-page">
        <SignTone tone={kin.tone} />
        <p className="tone-head">{i18n.t('tones:name')} {tt('.num_plural')} </p>
        <h2>{tt('.title')} {i18n.t('tones:name')} {tt('.title2')}</h2>
        <p className="about">
